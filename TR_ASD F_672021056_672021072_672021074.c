...
#include <stdlib.h>
#include <stdbool.h>
#include <windows.h>//mengubah warna texk bacground
...
struct node{//di dalam struct kami menampung
...
int top=0;
char un[50],pw[]; //ini kami gunakan untuk membuat pass dan
...
int masuk(){//jika kita masuk ke dalam program yang kita jumpai yaitu yang ini fungsi masuk
    system("Color CA");
    int x; double y;
    ...
    printf("\n\n\n");
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),2);// dengan kode haxedecimal nomor 2 yaitu
...
void loding(){//sesudah kita di login kita akan masuk ke dalam fungsi loading nnti akan di jelaskan oleh teman saya setalh di ran
    system("cls");
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),7);
    printf("\n\n\n\n\t\t\t\t\t\t\t LOADING...");
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),2);
